...
echo "── 2. Emulator safety gate ────────────────────────────"
if grep -q 'NODE_ENV === "production"' scripts/simulate-garmin.js 2>/dev/null; then
  pass "simulate-garmin.js has the production refusal gate"
else
  fail "simulate-garmin.js missing production gate"
fi
