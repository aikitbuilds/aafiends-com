...
export const AI4AA_ACCENT: Record<string, { text: string; bg: string; border: string; dot: string; grad: string }> = {
  ...
  blue: { text: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/40", dot: "bg-blue-500", grad: "from-[#7fb3a3] to-[#4cc07a]" },
  orange: { text: "text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/40", dot: "bg-orange-500", grad: "from-[#c2603f] to-[#e0a45c]" },
  violet: { text: "text-violet-400", bg: "bg-violet-500/10", border: "border-violet-500/40", dot: "bg-violet-500", grad: "from-[#a88fc4] to-[#7fb3a3]" },
  amber: { text: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/40", dot: "bg-amber-500", grad: "from-[#e0a45c] to-[#c2603f]" },
  emerald: { text: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/40", dot: "bg-emerald-500", grad: "from-[#4cc07a] to-[#7fb3a3]" },
          ...
          <div key={l.layer} className="flex flex-col items-center gap-1">
            <div className={`w-full rounded-2xl border ${c.border} bg-gradient-to-r ${c.grad} p-[1.5px] shadow-lg`}>
              <div className="flex items-center gap-4 rounded-2xl bg-[#0a0a0f] p-4 sm:p-5">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${c.grad} flex items-center justify-center text-[#f2efe6] shrink-0 shadow-md`}>
                  <Icon size={22} />
                </div>
                <div className="min-w-0">
                  <div className={`text-[10px] font-semibold ${c.text}`}>{l.layer}</div>
                  <div className="text-[#f2efe6] font-semibold">{l.tool}</div>
                  <div className="text-[#b8b4a6] text-xs leading-snug mt-0.5">{l.task}</div>
                </div>
              </div>
            </div>
            {i < layers.length - 1 && <ArrowDown size={18} className="text-[#7d7a70]" />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {TOOL_COMPARISON.map((t) => {
        ...
        return (
          <div key={t.tool} className={`rounded-2xl border ${c.border} bg-[#141814] overflow-hidden flex flex-col`}>
            <div className={`bg-gradient-to-r ${c.grad} px-5 py-4 flex items-center gap-3`}>
              <div className="w-10 h-10 rounded-xl bg-black/25 text-[#f2efe6] flex items-center justify-center backdrop-blur">
                <Icon size={18} />
              </div>
              <div>
                <div className="text-[#f2efe6] font-semibold tracking-tight text-sm">{t.tool}</div>
                <div className="text-[#f2efe6]/80 text-[10px] font-bold">{t.role}</div>
              </div>
            </div>
            <div className="p-5 flex flex-col gap-3 flex-1">
              <div className="text-xs text-[#b8b4a6] leading-relaxed"><span className="text-[#7d7a70]">Best for:</span> {t.bestFor}</div>
              <div className="text-xs text-[#b8b4a6] leading-relaxed"><span className="text-[#7d7a70]">2026 standout:</span> {t.standout}</div>
              <div className="flex gap-2 mt-auto pt-2 text-[10px] font-mono">
                <span className={`px-2 py-1 rounded ${c.bg} ${c.text}`}>Code: {t.coding.split(" ")[0]}</span>
                <span className="px-2 py-1 rounded bg-[#141814] text-[#b8b4a6]">Web: {t.web.split(" ")[0]}</span>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4">
        <div className="text-[10px] font-semibold text-red-400 mb-1.5">✗ Weak prompt</div>
        <p className="text-[#b8b4a6] text-sm font-mono">&ldquo;{PROMPT_ANATOMY.weak}&rdquo;</p>
      </div>

      {/* Strong — color-coded segments */}
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4">
        <div className="text-[10px] font-semibold text-emerald-400 mb-3">✓ Strong prompt — every part has a job</div>
...
export function BeforeAfterBars({ metric }: { metric: CaseMetric }) {
  ...
  return (
    <div ref={ref} className="rounded-2xl border border-[#1d231d] bg-[#141814] p-5 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="text-sm font-semibold text-[#f2efe6] tracking-tight flex items-center gap-2">
          <Zap size={15} className="text-amber-400" /> {metric.label}
        </div>
        <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded-full px-2.5 py-1">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <div className="flex justify-between text-[10px] font-bold mb-1">
            <span className="text-[#7d7a70]">Before AI</span>
            <span className="text-[#b8b4a6]">{metric.before} {metric.unit}</span>
          </div>
          <div className="h-3 rounded-full bg-[#141814] overflow-hidden">
        ...
        <div>
          <div className="flex justify-between text-[10px] font-bold mb-1">
            ...
            <span className="text-[#f2efe6]">{metric.after} {metric.unit}</span>
          </div>
          <div className="h-3 rounded-full bg-[#141814] overflow-hidden">
...
export function WeeklyRhythm() {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex w-full h-4 rounded-full overflow-hidden border border-[#1d231d]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {WEEKLY_RHYTHM.map((s, i) => (
          <div key={s.segment} className="flex items-start gap-2 text-xs">
            ...
            <span className="text-[#b8b4a6]"><span className="text-[#f2efe6] font-bold">{s.segment}</span> <span className="text-[#7d7a70]">· {s.minutes}m</span> — {s.purpose}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="flex flex-col gap-2">
      {CAMERA_MILESTONES.map((m) => {
        ...
        return (
          <div
            key={m.week}
            className={`flex items-center gap-3 rounded-xl border p-3 transition-colors ${active ? "border-amber-500/50 bg-gradient-to-r from-amber-500/15 to-transparent" : "border-[#1d231d] bg-[#141814]"}`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-xs font-semibold ${active ? "bg-gradient-to-br from-[#e0a45c] to-[#c2603f] text-black" : "bg-[#141814] text-[#b8b4a6]"}`}>
              {m.week}
            </div>
            <div className="min-w-0">
              <div className="text-[#f2efe6] text-xs font-bold flex items-center gap-1.5"><Camera size={12} className="text-[#7d7a70]" /> {m.skill}</div>
              <div className="text-[#7d7a70] text-[11px] leading-snug">{m.deliverable}</div>
            </div>
          </div>
        );
      })}
